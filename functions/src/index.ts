...
function location(assistant) {
  ...
  if (assistant.isPermissionGranted()) {
    ...
    const distance = getDistanceFromLatLonInKm(deviceCoordinates.latitude, deviceCoordinates.longitude, 52.508338, 6.0902274);
  ...
  admin.database().ref('session/whitelist').once('value', ((userData) => {
    admin.database().ref('session/blacklist').once('value', ((blacklistData) => {
      ...
      if (check === 2) {
        ...
        const preciseLocationPermission = assistant.SupportedPermissions.DEVICE_PRECISE_LOCATION
